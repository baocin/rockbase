...
fn scratch(name: &str) -> std::path::PathBuf {
    std::env::temp_dir().join(format!("rockbase_test_{}_{name}", uuid::Uuid::new_v4().simple()))
}
...
async fn backups() {
    ...
    for e in std::fs::read_dir(std::env::temp_dir()).unwrap().flatten() {
        if e.file_name().to_string_lossy().starts_with("rockbase_backup_") {
    ...
    let (s, _) = call(
        &app, "POST", "/api/collections/users/records", Some(ADMIN),
        Some(json!({"email": "bk@cave.dev", "password": "clubrock1"})),
    )
    .await;
    assert_eq!(s, StatusCode::OK);
    let (s, v) = call(
        &app, "POST", "/api/collections/users/auth-with-password", None,
    ...
    let (s, _) = call(
        &app, "POST", "/api/collections", Some(ADMIN),
        Some(json!({"name": "notes", "schema": [{"name": "body", "type": "text"}]})),
    )
    .await;
    assert_eq!(s, StatusCode::OK);
    let (s, _) = call(&app, "POST", "/api/collections/notes/records", Some(ADMIN), Some(json!({"body": "hi"}))).await;
    assert_eq!(s, StatusCode::OK);
    ...
    let n: i64 = conn.query_row("SELECT COUNT(*) FROM records", [], |r| r.get(0)).unwrap();
    assert_eq!(n, 2); // 1 user + 1 note
    let has: i64 = conn
        .query_row("SELECT COUNT(*) FROM _collections WHERE name = 'notes'", [], |r| r.get(0))
        .unwrap();
    assert_eq!(has, 1);
    drop(conn);
    ...
    let leftovers: Vec<_> = std::fs::read_dir(std::env::temp_dir())
        .unwrap()
        .flatten()
        .filter(|e| e.file_name().to_string_lossy().starts_with("rockbase_backup_"))
        .collect();
...
async fn full_flow() {
    ...
    let (s, _) = call(&app, "POST", "/api/collections", None, Some(json!({"name": "posts"}))).await;
    assert_eq!(s, StatusCode::UNAUTHORIZED);
    ...
    let (s, _) = call(&app, "POST", "/api/collections/posts/records", None, Some(json!({"title": "x"}))).await;
    assert_eq!(s, StatusCode::UNAUTHORIZED);
    ...
    let (s, _) = call(&app, "POST", "/api/collections/posts/records", Some(&bearer), Some(json!({"views": 1}))).await;
    assert_eq!(s, StatusCode::BAD_REQUEST);
    let (s, _) = call(&app, "POST", "/api/collections/posts/records", Some(&bearer), Some(json!({"title": 5}))).await;
    assert_eq!(s, StatusCode::BAD_REQUEST);
    let (s, _) = call(&app, "POST", "/api/collections/posts/records", Some(&bearer), Some(json!({"title": "a", "nope": 1}))).await;
    assert_eq!(s, StatusCode::BAD_REQUEST);

    // create two, filter, sort
    let (s, first) = call(&app, "POST", "/api/collections/posts/records", Some(&bearer), Some(json!({"title": "first", "views": 10}))).await;
    assert_eq!(s, StatusCode::OK);
    let (s, _) = call(&app, "POST", "/api/collections/posts/records", Some(&bearer), Some(json!({"title": "second", "views": 99}))).await;
    assert_eq!(s, StatusCode::OK);

    let (s, v) = call(&app, "GET", "/api/collections/posts/records?filter=views%3E50", None, None).await;
    assert_eq!(s, StatusCode::OK);
    ...
    let (s, v) = call(&app, "GET", "/api/collections/posts/records?sort=-views", None, None).await;
    assert_eq!(s, StatusCode::OK);
    assert_eq!(v["items"][0]["title"], "second");
    ...
    let (s, v) = call(&app, "PATCH", &format!("/api/collections/posts/records/{id}"), Some(&bearer), Some(json!({"views": 11}))).await;
    assert_eq!(s, StatusCode::OK);
    assert_eq!(v["views"], 11);
    assert_eq!(v["title"], "first");

    let (s, _) = call(&app, "DELETE", &format!("/api/collections/posts/records/{id}"), Some(&bearer), None).await;
    assert_eq!(s, StatusCode::OK);
    let (s, _) = call(&app, "GET", &format!("/api/collections/posts/records/{id}"), None, None).await;
...
async fn query_extras_and_patch_null() {
    ...
    for (title, views) in [("b", 5), ("a", 5), ("c", 9)] {
        let (s, v) = call(&app, "POST", "/api/collections/posts/records", Some(&bearer), Some(json!({"title": title, "views": views}))).await;
        assert_eq!(s, StatusCode::OK);
        if title == "b" {
    ...
    let (s, v) = call(&app, "GET", "/api/collections/posts/records?sort=-views,title", None, None).await;
    assert_eq!(s, StatusCode::OK);
    ...
    for uri in ["/api/collections/posts/records?sort=views,bad-seg!", "/api/collections/posts/records?sort=views,"] {
    ...
    let (s, v) = call(&app, "GET", "/api/collections/posts/records?fields=title", None, None).await;
    assert_eq!(s, StatusCode::OK);
    for item in v["items"].as_array().unwrap() {
        let keys: Vec<&String> = item.as_object().unwrap().keys().collect();
        assert_eq!(keys.len(), 2, "{item}");
        assert!(item.get("id").is_some() && item.get("title").is_some(), "{item}");
    }

    // 4. bad fields segment -> 400
    let (s, v) = call(&app, "GET", "/api/collections/posts/records?fields=title;views", None, None).await;
    assert_eq!(s, StatusCode::BAD_REQUEST);
    assert_eq!(v["message"], "bad fields");

    // 5. skipTotal=1: -1 sentinels, pagination still works
    let (s, v) = call(&app, "GET", "/api/collections/posts/records?skipTotal=1&perPage=2", None, None).await;
    assert_eq!(s, StatusCode::OK);
    assert_eq!(v["totalItems"], -1);
    ...
    let (s, v) = call(&app, "GET", "/api/collections/posts/records?filter=views%3E4&skipTotal=1&sort=-views&fields=title", None, None).await;
    assert_eq!(s, StatusCode::OK);
    assert_eq!(v["totalItems"], -1);
    ...
    let (s, v) = call(&app, "PATCH", &format!("/api/collections/posts/records/{id_b}"), Some(&bearer), Some(json!({"title": null}))).await;
    assert_eq!(s, StatusCode::BAD_REQUEST);
    assert_eq!(v["message"], "field 'title' is required");
    let (s, v) = call(&app, "GET", &format!("/api/collections/posts/records/{id_b}"), None, None).await;
    assert_eq!(s, StatusCode::OK);
    assert_eq!(v["title"], "b");

    // 8. PATCH null on a non-required field -> 200
    let (s, v) = call(&app, "PATCH", &format!("/api/collections/posts/records/{id_b}"), Some(&bearer), Some(json!({"views": null}))).await;
    assert_eq!(s, StatusCode::OK);
...
async fn auth_refresh_identity_and_existence() {
    ...
    let (s, v) = call(&app, "POST", "/api/collections/users/auth-refresh", Some(&bearer), None).await;
    assert_eq!(s, StatusCode::OK);
    ...
    let (s, _) = call(&app, "POST", "/api/collections/posts/records", Some(&fresh_bearer), Some(json!({"title": "x"}))).await;
    assert_eq!(s, StatusCode::OK);

    // 3. no / garbage token -> 401
    let (s, v) = call(&app, "POST", "/api/collections/users/auth-refresh", None, None).await;
    assert_eq!(s, StatusCode::UNAUTHORIZED);
    assert_eq!(v["code"], 401);
    let (s, _) = call(&app, "POST", "/api/collections/users/auth-refresh", Some("Bearer notatoken"), None).await;
    assert_eq!(s, StatusCode::UNAUTHORIZED);

    // 4. admin token rejected (not an auth record)
    let (s, _) = call(&app, "POST", "/api/collections/users/auth-refresh", Some(ADMIN), None).await;
    assert_eq!(s, StatusCode::UNAUTHORIZED);
    ...
    .await;
    assert_eq!(s, StatusCode::OK);
    let (s, v) = call(&app, "POST", "/api/collections/staff/auth-refresh", Some(&bearer), None).await;
    assert_eq!(s, StatusCode::UNAUTHORIZED);
    assert_eq!(v["message"], "token is for another collection");

    // 7. password change via PATCH: old login dies, new works
    let (s, _) = call(&app, "PATCH", &format!("/api/collections/users/records/{uid}"), Some(&bearer), Some(json!({"password": "newpass99"}))).await;
    assert_eq!(s, StatusCode::OK);
    let (s, _) = call(
        &app, "POST", "/api/collections/users/auth-with-password", None,
        Some(json!({"identity": "og@cave.dev", "password": "clubrock1"})),
    ...
    let (s, _) = call(&app, "DELETE", &format!("/api/collections/users/records/{uid}"), Some(ADMIN), None).await;
    assert_eq!(s, StatusCode::OK);
    let (s, _) = call(&app, "POST", "/api/collections/posts/records", Some(&bearer), Some(json!({"title": "y"}))).await;
    assert_eq!(s, StatusCode::UNAUTHORIZED);
    let (s, _) = call(&app, "POST", "/api/collections/users/auth-refresh", Some(&bearer), None).await;
    assert_eq!(s, StatusCode::UNAUTHORIZED);
}
